...
pub const SPIGOT_BUILD_TOOLS_API_URL: &str = "https://hub.spigotmc.org/jenkins/job/BuildTools/lastSuccessfulBuild/api/json";
...
pub fn get_build_tools_path() -> PathBuf {
    std::path::Path::new(".").join(&BUILD_TOOLS_DIR).join(&BUILD_TOOLS_JAR)
}

pub fn get_build_tools_metadata_path() -> PathBuf {
    std::path::Path::new(".").join(&BUILD_TOOLS_DIR).join(&BUILD_TOOLS_METADATA)
}